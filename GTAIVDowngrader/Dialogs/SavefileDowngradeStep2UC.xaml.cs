...
namespace GTAIVDowngrader.Dialogs {
    public partial class SavefileDowngradeStep2UC : UserControl {
        ...
        private void WebClient_UploadProgressChanged(object sender, UploadProgressChangedEventArgs e)
        {
            ...
            string bytesReceived = Helper.GetExactFileSize2(e.BytesReceived);
            string totalBytesToReceived = Helper.GetExactFileSize2(e.TotalBytesToReceive);
        ...
        private void WebClient_UploadFileCompleted(object sender, UploadFileCompletedEventArgs e)
        {
            ...
            if (e.Cancelled)
                return;
            if (e.Error != null) {
                MainFunctions.Notification.ShowNotification(NotificationType.Error, 6500, string.Format("{0} - Upload error... Continuing", saveFileName), e.Error.Message, saveFileName);
                MainFunctions.AddLogItem(LogType.Error, string.Format("An error occured while trying to upload {0}. Details: {1}", saveFileName, e.Error.Message));
                ...
                if (saveFilesUploadQueue.Count != 0) {
                    ...
                    // Log
                    MainFunctions.AddLogItem(LogType.Info, string.Format("(1) - Uploading next file {0}", nextSaveFileToBeUploaded));
                }
                else {
                    ...
                    // Log
                    MainFunctions.AddLogItem(LogType.Info, string.Format("(1) - Uploading complete! Started downloading file {0}", nextSaveFileToBeDownloaded.FileName));
                }
            ...
            for (int i = 0; i < responseLines.Length; i++) {
                ...
                // Checks
                if (!isValidSaveGame) {
                    MainFunctions.Notification.ShowNotification(NotificationType.Warning, 3000, "Invalid file", string.Format("{0} is not a valid GTA IV save game file.", saveFileName));
                    break;
                }
                if (!isGTAIVSaveGame) {
                    MainFunctions.Notification.ShowNotification(NotificationType.Warning, 3000, "Wrong game", string.Format("{0} is not a valid GTA IV save game file.", saveFileName));
                    break;
                }
            ...
            if (slotStrRaw == "00") {
                ...
                slot = Helper.ParseExtension.Parse(slotStrRaw, 0);
            }

            // Add to download queue if uploaded save game was valid
            if (isValidSaveGame && isGTAIVSaveGame) saveFilesDownloadQueue.Enqueue(new SaveFileDownload(saveFileName, saveGameID, slot));
            ...
            if (saveFilesUploadQueue.Count != 0) {
                ...
                // Log
                MainFunctions.AddLogItem(LogType.Info, string.Format("(2) - Uploading next file {0}", nextSaveFileToBeUploaded));
            }
            else {
                UpdateStatusText("Uploading complete! Continuing with downloading converted saves...");
                ...
                // Log
                MainFunctions.AddLogItem(LogType.Info, string.Format("(2) - Uploading complete! Started downloading file {0}", nextSaveFileToBeDownloaded.FileName));
            }
        }
        ...
        private void WebClient_DownloadFileCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
            ...
            if (e.Cancelled)
                return;
            if (e.Error != null) {
                MainFunctions.Notification.ShowNotification(NotificationType.Error, 6500, string.Format("{0} - Download error... Continuing", saveFileName), e.Error.Message, saveFileName);
                MainFunctions.AddLogItem(LogType.Error, string.Format("An error occured while trying to download {0}. Details: {1}", saveFileName, e.Error.Message));
                ...
                if (saveFilesDownloadQueue.Count != 0) {
                    ...
                    // Log
                    MainFunctions.AddLogItem(LogType.Info, string.Format("(1) - Downloading next file {0}", nextSaveFileToBeDownloaded.FileName));
                }
                else {
                    ...
                    // Log
                    MainFunctions.AddLogItem(LogType.Info, "(1) - Coverting and downloading saves completed!");
                }
            ...
            if (saveFilesDownloadQueue.Count != 0) {
                ...
                // Log
                MainFunctions.AddLogItem(LogType.Info, string.Format("(2) - Downloading next file {0}", nextSaveFileToBeDownloaded.FileName));
            }
            else {
                selectedSaveFiles = null;
                ChangeSelectedFilesTextVisibility(false);
                ...
                // Log
                MainFunctions.AddLogItem(LogType.Info, "(2) - Coverting and downloading saves completed!");
            }
        }
        #endregion
        ...
        private void DowngradeButton_Click(object sender, RoutedEventArgs e)
        {
            try {
                if (selectedSaveFiles == null || selectedSaveFiles.Length == 0) {
                    MainFunctions.Notification.ShowNotification(NotificationType.Warning, 3000, "No save file(s) selected", "Please select atleast one save file.");
                    return;
                }
                ...
                // Log
                MainFunctions.AddLogItem(LogType.Info, "- - - GTASnP Save File Downgrading - - -");
                MainFunctions.AddLogItem(LogType.Info, string.Format("Starting uploading {0}", nextSaveFileToBeUploaded));
            }
            catch (Exception ex) {
                MainFunctions.AddLogItem(LogType.Info, "- - - GTASnP Save File Downgrading - - -");
                MainFunctions.AddLogItem(LogType.Info, string.Format("Failed to start save file downgrade... Details: {0}", ex.Message));
            }
        }
